...
"""
import verl
print("[DEBUG] verl path:", verl.__file__)
import json
import os
import uuid
...
def compute_advantage(
    ...
    norm_adv_by_std_in_grpo: bool = True,
    config: Optional[AlgoConfig] = None,
    reason_direction: Optional[torch.Tensor] = None,
) -> DataProto:
    ...
    # '''
    # 改变1
    # 在 elif adv_estimator == AdvantageEstimator.GRPO: 这个分支里，先判断：
    #     if config.g2rl.enable:
    #         调 G2RL shaping
    #         改写 data.batch["token_level_rewards"]
    #     然后再调用原来的 compute_grpo_outcome_advantage(...)
    # '''
        ...
        # 改变，新加------------------------------------------
        # ===== G2RL shaping =====
        if config is not None and config.get("g2rl", {}).get("enable", False):
            from verl.trainer.ppo.g2rl import apply_g2rl_reward_shaping

            assert "g2rl_seq_feat" in data.batch, (
                "G2RL requires g2rl_seq_feat from actor.compute_log_prob"
            )

            use_reason_subgroup = config.get("g2rl", {}).get("reason_enable", False)
            if use_reason_subgroup:
                assert "reason_hs_feat" in data.batch, "reason_enable=True requires reason_hs_feat from actor.compute_log_prob"
                assert reason_direction is not None, "reason_enable=True requires loaded reason_direction"

            shaped_rewards, g2rl_metrics = apply_g2rl_reward_shaping(
                token_level_rewards=data.batch["token_level_rewards"],
                response_mask=grpo_calculation_mask,
                seq_feat=data.batch["g2rl_seq_feat"],
                index=data.non_tensor_batch["uid"],
                reason_hs_feat=data.batch["reason_hs_feat"] if use_reason_subgroup else None,
                reason_direction=reason_direction,
                use_reason_subgroup=use_reason_subgroup,
                lambda_pos=config.g2rl.get("lambda_pos", 0.5),
                lambda_neg=config.g2rl.get("lambda_neg", 0.5),
                reward_clip=config.g2rl.get("reward_clip", 3.0),
            )

            data.batch["token_level_rewards"] = shaped_rewards
            data.meta_info.setdefault("metrics", {})
            data.meta_info["metrics"].update(g2rl_metrics)
            print(
                "[G2RL trainer] reward_raw_mean =",
                g2rl_metrics["g2rl/reward_raw_mean"],
                "reward_shaped_mean =",
                g2rl_metrics["g2rl/reward_shaped_mean"],
            )

            if "g2rl/nu_final_mean" in g2rl_metrics:
                print(
                    "[G2RL trainer] nu_all_mean =",
                    g2rl_metrics["g2rl/nu_all_mean"],
                    "nu_reason_mean =",
                    g2rl_metrics["g2rl/nu_reason_mean"],
                    "nu_final_mean =",
                    g2rl_metrics["g2rl/nu_final_mean"],
                )
            else:
                print("[G2RL trainer] nu_mean =", g2rl_metrics["g2rl/nu_mean"])
        # 改变，新加------------------------------------------
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.g2rl_reason_direction = None
        self.g2rl_reason_best_layer = None
        self._maybe_init_reason_direction()

    def _maybe_init_reason_direction(self):
        g2rl_cfg = self.config.algorithm.get("g2rl", {})
        if not g2rl_cfg.get("enable", False):
            return
        if not g2rl_cfg.get("reason_enable", False):
            return

        direction_pt = g2rl_cfg.get("reason_direction_pt", None)
        assert direction_pt is not None, "reason_enable=True requires algorithm.g2rl.reason_direction_pt"

        # PyTorch 2.6: default weights_only=True may fail for our saved dict
        data = torch.load(direction_pt, map_location="cpu", weights_only=False)

        assert "directions" in data and "best_layer" in data, \
            "direction pt must contain 'directions' and 'best_layer'"
        ...
        self.g2rl_reason_direction = direction.float().cpu()
        self.g2rl_reason_best_layer = best_layer

        print(
            "[G2RL trainer] loaded reason direction:",
            direction_pt,
            "best_layer =",
            self.g2rl_reason_best_layer,
            "shape =",
            tuple(self.g2rl_reason_direction.shape),
        )
    ...
    '''
    改变2:
    这个函数现在会调用 self.actor_rollout_wg.compute_log_prob(batch) 来拿 old_log_probs 和 entropys。
    这里要额外把 return_g2rl_seq_feat=True 塞进 meta_info，并把 worker 返回的 g2rl_seq_feat 合并进 batch。
    '''
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if bypass_recomputing_logprobs:  # Use `rollout_log_probs`
                        ...
                        # 改变，新增------------------------
                        # >>>>>>> G2RL add start
                        #batch.meta_info["return_g2rl_seq_feat"] = self.config.algorithm.get("g2rl", {}).get("enable", False)
                        
                        g2rl_cfg = self.config.algorithm.get("g2rl", {})
                        batch.meta_info["return_g2rl_seq_feat"] = g2rl_cfg.get("enable", False)
                        batch.meta_info["return_reason_hs_feat"] = (
                            g2rl_cfg.get("enable", False) and g2rl_cfg.get("reason_enable", False)
                        )
                        batch.meta_info["reason_best_layer"] = self.g2rl_reason_best_layer
                        # reason4_2_match: direction-aware alpha_t weighting on Phi only
                        batch.meta_info["g2rl_alpha_direction"] = self.g2rl_reason_direction
                        batch.meta_info["g2rl_alpha_tau"] = float(g2rl_cfg.get("tau_alpha", 1.0))
                        # <<<<<<< G2RL add end
                        # 改变，新增------------------------
                        ...
                        with marked_timer("old_log_prob", timing_raw, color="blue"):
                            ...
                            # 改变，新增测试-------
                            print("old_log_prob keys:", list(old_log_prob.batch.keys()))
                            if "g2rl_seq_feat" in old_log_prob.batch:
                                print("g2rl_seq_feat shape:", old_log_prob.batch["g2rl_seq_feat"].shape)
                            # 改变，新增测试-------
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        batch = compute_advantage(
                            ...
                            reason_direction=self.g2rl_reason_direction,
                        )
