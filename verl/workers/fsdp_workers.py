...
"""
import verl
print("[WORKER DEBUG] verl path:", verl.__file__)
import datetime
import json
import logging
import os
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    '''
    改变：
    改 compute_log_prob(...)，让它在原来返回 old_log_probs 的同时，可选返回 g2rl_seq_feat。当前函数已经是：
        读 data.meta_info
        调 self.actor.compute_log_prob(...)
        把 outputs["log_probs"] / outputs["entropys"] 打包回 DataProto。
    所以这里最适合加：
        data.meta_info["return_g2rl_seq_feat"] = True/False
        如果为 True，则 outputs 里多取一个 seq_feat
        放进 tensors["g2rl_seq_feat"]
    '''
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        # 改变，新增--------------->>>>>>> G2RL add start
        return_g2rl_seq_feat = data.meta_info.get("return_g2rl_seq_feat", False) and (not is_lora)
        return_reason_hs_feat = data.meta_info.get("return_reason_hs_feat", False) and (not is_lora)
        reason_best_layer = data.meta_info.get("reason_best_layer", None)
        # reason4_2_match: direction-aware alpha_t weighting
        g2rl_alpha_direction = data.meta_info.get("g2rl_alpha_direction", None)
        g2rl_alpha_tau = float(data.meta_info.get("g2rl_alpha_tau", 1.0))
        # 改变，新增---------------<<<<<<< G2RL add end
        ...
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                ...
                # 改变，新增----------需要参数return_g2rl_seq_feat，否则 smoke test 根本不会触发。你现在上传版还没传这个参数
                outputs = self.actor.compute_log_prob(
                                                        data=data,
                                                        calculate_entropy=calculate_entropy,
                                                        return_g2rl_seq_feat=return_g2rl_seq_feat,
                                                        return_reason_hs_feat=return_reason_hs_feat,
                                                        reason_best_layer=reason_best_layer,
                                                        alpha_direction=g2rl_alpha_direction,
                                                        alpha_tau=g2rl_alpha_tau,
                                                    )
                print("[FSDP worker] actor outputs keys:", list(outputs.keys()))
                if "g2rl_seq_feat" in outputs:
                    print("[FSDP worker] g2rl_seq_feat shape:", outputs["g2rl_seq_feat"].shape)
                if "reason_hs_feat" in outputs:
                    print("[FSDP worker] reason_hs_feat shape:", outputs["reason_hs_feat"].shape)
                if "g2rl_seq_feat" in outputs:
                    print("[FSDP worker] g2rl_seq_feat shape:", outputs["g2rl_seq_feat"].shape)
            ...
            # 改变，新增--------------->>>>>>> G2RL add start
            if return_g2rl_seq_feat:
                assert "g2rl_seq_feat" in outputs, "return_g2rl_seq_feat=True but actor did not return g2rl_seq_feat"
                tensors["g2rl_seq_feat"] = outputs["g2rl_seq_feat"]
            if return_reason_hs_feat:
                assert "reason_hs_feat" in outputs, "return_reason_hs_feat=True but actor did not return reason_hs_feat"
                tensors["reason_hs_feat"] = outputs["reason_hs_feat"]
            # 改变，新增---------------<<<<<<< G2RL add end
